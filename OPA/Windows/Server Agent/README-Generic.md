...
## Overview
This PowerShell script installs the **Okta Privileged Access (OPA) / ScaleFT Server Tools** (server agent service `sftd`) on **Windows Server** by downloading the **latest version** from Okta’s official Windows `server-tools` repository.
