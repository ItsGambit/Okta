<#
.SYNOPSIS
  SCCM VARIANT: Install latest Okta Privileged Access (OPA) / ScaleFT Server Tools (sftd) on Windows Server.
...
# SCCM: allow token to come from a machine env var (recommended) if not passed.
# Example: set via task sequence step: setx OKTA_OPA_ENROLLMENT_TOKEN "<token>" /M
