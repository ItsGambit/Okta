<#
.SYNOPSIS
  INTUNE VARIANT: Install latest Okta Privileged Access (OPA) / ScaleFT Server Tools (sftd) on Windows Server.
...
# Enrollment token handling
# Intune runs scripts non-interactively. This variant defaults to non-interactive unless a real user session is detected.
$isInteractive = [Environment]::UserInteractive -and -not $NonInteractive
if (-not $isInteractive -and -not $NonInteractive) {
  $NonInteractive = $true
}

# In Intune, prefer pulling the token from an environment variable if not supplied.
# Set this as a SYSTEM environment variable or provide -EnrollmentToken in the install command line.
if (-not $EnrollmentToken) {
  $EnrollmentToken = [Environment]::GetEnvironmentVariable("OKTA_OPA_ENROLLMENT_TOKEN", "Machine")
}

if (-not $EnrollmentToken) {
  if (-not $isInteractive) {
    Write-Log "Non-interactive mode requires -EnrollmentToken or machine env var OKTA_OPA_ENROLLMENT_TOKEN." "ERROR"
