...
parse_args() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --nis-domain) require_option_value "$1" "${2:-}"; NIS_DOMAIN="$2"; shift 2 ;;
      --master-fqdn) require_option_value "$1" "${2:-}"; MASTER_FQDN="$2"; shift 2 ;;
      --master-ip) require_option_value "$1" "${2:-}"; MASTER_IP="$2"; shift 2 ;;
      --allowed-cidr) require_option_value "$1" "${2:-}"; ALLOWED_CIDR="$2"; shift 2 ;;
      --ypserv-port) require_option_value "$1" "${2:-}"; YPSERV_PORT="$2"; shift 2 ;;
      --ypxfrd-port) require_option_value "$1" "${2:-}"; YPXFRD_PORT="$2"; shift 2 ;;
      --yppasswdd-port) require_option_value "$1" "${2:-}"; YPPASSWDD_PORT="$2"; shift 2 ;;
...
configure_hostname_and_hosts() {
  ...
  local short_name tmp_hosts
  short_name="${MASTER_FQDN%%.*}"
  tmp_hosts="$(mktemp)"
  awk -v ip="$MASTER_IP" -v fqdn="$MASTER_FQDN" -v short="$short_name" '
    BEGIN { written=0 }
    {
      matched=($1 == ip)
      for (i=2; i<=NF; i++) {
